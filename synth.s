...
start:
    ...
    ld a, 0x00
    out0 (0x36), a
    # Disable memory wait state generation
    in0 a, (DCNTL)
    and 0x3f
    out0 (DCNTL), a
    ...
    # Write welcome text/instructions to the terminal
    ld hl, terminaltext_welcome
    call terminal_print
    ld hl, terminaltext_prompt
    call terminal_print

    # Enable PRT interrupts
    ld a, PRT_ENABLED
    out0 (PRT_TCR), a
...
#
# The channel number is stored left-shifted by one for 
# efficiency reasons - all usage of this value needs this 
# form (16-bit lookup tables).  Interrupts must be disabled 
# when this is called to avoid double-swapping.
############################################################
set_channel:
...
############################################################
# Play the MIDI note stored in the accumulator
#
# This translates between the MIDI note and the PRT/divisor
# setup using a lookup table.  It is effectively the same as
# the respective part in the network packet handler, but 
# that is inlined for speed.  Interrupts must be disabled
# before calling this.
############################################################
set_note:
    # Convert the note to a 4-byte-aligned offset
    ld l, a
    ld h, 0x00
    sla l
    sla l
...
############################################################
# Interrupt vector table, aligned to 5-bit boundary so that 
# the 9 bits of I + IL are the base of the vector table
############################################################
.align 5
interrupts:
    .int int_int1
...
#
# Uses a "lockout" flag to stop the repeating of an action
# when the key is held down (which happens every time 
# because pressing a key is a lot slower than handling the
# keypress).
############################################################
keypad_lockout:
    .byte 0x00
int_int2:
    di
    # Check if the lockout is set, if so then do nothing as 
    # this keypress has already been handled
    ld a, (keypad_lockout)
    cp 0x00
    jr z, 1f
    # Pop the stack so it doesn't become full of return 
    # addresses for repeated interrupts
    pop hl
    jr 2f

    # If the interrupt has not been handled, set the lockout
1:  ld a, 0xff
    ld (keypad_lockout), a
    # Get the keypad value and set the channel
    call keypad_getbyte
    call set_channel

    # Re-enable interrupts and wait a few instructions to 
    # see if the interrupts are still being generated - if
    # they are an interrupt will happen before the lockout 
    # is reset.  When the keypad finally stops interrupting,
    # the lockout will be reset ready for the next keypress.
2:  ei
    nop
    nop
    nop
...
#
# Uses only the shadow registers, so that the interrupt 
# handler finishes quickly and can happen during other 
# interrupts without making a mess.
#
# Registers used for state - D'E', C'
############################################################
int_prt0:
    ...
    # Increment the low byte by the divisor amount
    ld a, e
    add a, c
    ld e, a
...
#
# There are two sections - one during which interrupts are 
# locked out, and those where interrupts are enabled.  Where 
# interrupts are disabled, the "shadow" registers are used, 
# carefully avoiding those used by the PRT handler.
############################################################
int_asci0:
    #
    # Critical section - interrupts disabled, shadow 
    # registers used
    #
    # Volume and pitch are H'L' respectively (rather than 
    # the order they are in incoming data) as a slight 
    # efficiency measure - the pitch needs to be used as the
    # low byte of a 16-bit add anyway.
    #
    # Registers used for state:
    #   H'      - Volume
    #   L'      - Pitch
    #   B'      - character counter
    #
    ...
    exx
    ### Effectively network_getchar, but inlined for speed
    # Reset CTRLA
...
#
# Implements an interactive terminal for certain actions.  
# Since the terminal is not part of essential operation, I 
# have not focussed on efficiency for it, so that efficiency
# can be achieved in the more critical areas.
############################################################
int_asci1:
    di
    # Preserve registers
    ...
    ### Piano mode handlers
    # EOF character (ctrl+d) to exit piano mode
    cp 0x04 ; jp z, command_eof
    # Increment octave offset
    cp '+' ; jr nz, 1f
    ld a, (piano_offset)
    add a, 0x0c
    jr nc, 0f
...
1:  cp '-' ; jr nz, 1f
    ...
    ld a, 0x00
0:  ld (piano_offset), a
    # Finished, goto cleanup
    jp 9f

    ### Any other key in piano mode gets mapped to a MIDI note
    # Perform the lookup
...
2:  ex af, af
    ...
    ### If not in piano mode, do a command
    ...
    # Goto cleanup with new prompt
    jp 8f

    ###
    ### Change the instrument independent of the channel
    ### 
command_i:
...
0:  ld l, a
    ...
    ld d, a
    exx
    # Finished, cleanup + new prompt
    jp 8f

    ###
    ### Change the channel independent of the instrument
    ###
command_c:
    # Echo the command back, plus a space
    call terminal_putchar
...
############################################################
# Piano key lookup - translate from ASCII value to a note
# in the first two octaves - the offset is used to convert 
# this into the current octaves
############################################################
piano_lookup:
    ...
    .byte 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
    .byte 0x00,0x00,0x0d,0x0f,0x00,0x12,0x14,0x16 #3
    .byte 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
    .byte 0x00,0x00,0x07,0x04,0x03,0x10,0x00,0x06 #4
    .byte 0x08,0x00,0x0a,0x00,0x00,0x0b,0x09,0x00
    .byte 0x00,0x0c,0x11,0x01,0x13,0x17,0x05,0x0e #5
    .byte 0x02,0x15,0x00,0x00,0x00,0x00,0x00,0x00
    .byte 0x00,0x00,0x07,0x04,0x03,0x10,0x00,0x06 #6
    .byte 0x08,0x00,0x0a,0x00,0x00,0x0b,0x09,0x00
    .byte 0x00,0x0c,0x11,0x01,0x13,0x17,0x05,0x0e #7
    .byte 0x02,0x15,0x00,0x00,0x00,0x00,0x00,0x00
